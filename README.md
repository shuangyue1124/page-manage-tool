...
不想敲命令？直接**双击项目根目录的 `manage-nav.bat`**，会弹出一个黑色窗口，显示菜单：
...
1. 双击 `manage-nav.bat`，等待菜单显示
2. 输入 `2` 后按回车 → 选择添加链接
3. 按提示输入分区编号（如 `2` = 网页管理）后回车
4. 输入标题（如 `Nginx Proxy Manager`）后回车
5. 输入 URL（如 `http://192.168.1.1:81`）后回车
6. 描述 / 标签 可直接回车跳过，置顶输入 `n` 后回车
7. 看到 `[OK] 已添加` 后，输入 `7` 按回车 → 自动 git 提交并推送
8. 等 Cloudflare Pages 自动构建完成，网页即更新（约几十秒）

修改（`3`）和删除（`4`）流程类似：按编号选链接 → 改字段（回车=保留，输入 `-` 清空）→ 最后记得选 `7` 推送。

> 提示：想更顺手，可把 `manage-nav.bat` 右键"发送到 → 桌面快捷方式"，然后重命名。若窗口内中文显示乱码，检查系统区域设置或在设置里启用 Windows Terminal 作为默认终端。
